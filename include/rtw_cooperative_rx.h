 ...
 *
 * Cooperative RX Diversity Mode — Open Network Only
 *
 ...
 *
 * This is the minimal open-network variant: no encryption support, no
 * kernel crypto transforms, no CAM mirroring.
 *
 ...
 *
 * Open-network-only: no crypto transforms, no CAM mirroring.
 */
struct cooperative_rx_group {
	...
	/*
	 * Tuning constants. PENDING_BASE scales with the number of
	 * active helpers. Must leave enough headroom in the primary's
	 * 256-frame pool (NR_RECVFRAME) for its USB recv pipeline.
	 * With drop_primary=1, all traffic goes through the pending
	 * queue, so this is the binding constraint.
	 */
#define COOP_PENDING_BASE	48	/* per-helper base allocation */
#define COOP_BATCH_SIZE		64	/* frames per tasklet run */
#define COOP_RCU_BATCH		16	/* synchronize_rcu every N removals */
...
/* Dynamic pending-queue limit: scales with number of helpers */
static inline int coop_pending_max(struct cooperative_rx_group *grp)
{
	return COOP_PENDING_BASE * (1 + READ_ONCE(grp->num_helpers));
}
